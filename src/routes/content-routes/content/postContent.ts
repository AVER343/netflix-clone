...
Content.post('*/',async(req,res)=>{
        try{
          let {films,series} =seedDatabase();
          let films_1 = films.map(e=>({...e,genre:[e.genre],type:'films'}))
          let series_1 = series.map(e=>({...e,genre:[e.genre],type:'series'}))
          let data = [...films_1,...series_1]
          let query = `INSERT INTO CONTENT_TABLE(data) VALUES `
          let args:any=[]
          data.map((e,i)=>{
            if(i!=data.length-1)
            {
          ...
          let response = await Server.pool.query(query,args);
          res.send({response})
        }
        catch(e){
          console.log({e})
