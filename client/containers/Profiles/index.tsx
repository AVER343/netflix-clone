...
function SelectProfileContainer({ user, setProfile }) {
    let {user_custom_properties}=user
    useEffect(()=>{
    ...
    <>
      ...
      <Profiles>
        ...
        <Profiles.List>
          <Profiles.User
            onClick={() =>{
               setProfile({ displayName: user.username, photoURL: user_custom_properties?.photoURL })
               window?.localStorage.setItem('profile',JSON.stringify({ displayName: user.username, photoURL: user_custom_properties?.photoURL }))
              }}
            data-testid="user-profile"
          >
            <Profiles.Picture src={user_custom_properties?.photoURL} />
            <Profiles.Name>{user.username.slice(0,7)}</Profiles.Name>
          </Profiles.User>
        </Profiles.List>
      </Profiles>
    </>
